...
document.querySelectorAll(".item").forEach(img => {
    const originalSrc = img.getAttribute('data-original');  // Store original image
    const gifSrc = img.getAttribute("data-gif");  // Store GIF image
    const uniqueId = img.getAttribute("data-id");  // Get unique ID for each image

    // Handle mouse enter event (show GIF)
    img.addEventListener("mouseenter", () => {
        img.src = gifSrc;  // Change to GIF
        console.log(`Image ${uniqueId} hover: switched to GIF`);
    });

    // Handle mouse leave event (show original image)
    img.addEventListener("mouseleave", () => {
        img.src = originalSrc;  // Revert to original image
        console.log(`Image ${uniqueId} hover: reverted to original`);
    });
});
