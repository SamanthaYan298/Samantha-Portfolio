...
function updateGallery(imageUrl, selectedImage, galleryId) {
    const activeGallery = document.getElementById('gallery_' + galleryId);
    if (activeGallery) {
...
// Gallery Banner with Hover Handling
document.querySelectorAll(".item").forEach(img => {
    const originalSrc = img.getAttribute('data-original');
    const gifSrc = img.getAttribute("data-gif");
    const uniqueId = img.getAttribute("data-id");  // Get the unique identifier

    img.addEventListener("mouseenter", () => {
        // Only change the image if it's not the gallery image (optional safeguard)
        if (!isGalleryUpdating) {
            console.log(`Changing to GIF for image ${uniqueId}`);  // Optional: Log the unique ID for debugging
            img.src = gifSrc;  // Change to GIF on hover
        }
    });

    img.addEventListener("mouseleave", () => {
        // Only revert the image back if it's not the gallery image (optional safeguard)
        if (!isGalleryUpdating) {
            console.log(`Reverting to original for image ${uniqueId}`);  // Optional: Log the unique ID for debugging
            img.src = originalSrc;  // Revert to the original image when the mouse leaves
        }
    });
});
