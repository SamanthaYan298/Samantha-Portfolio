...
let isGalleryUpdating = false;

function openGallery(id) {
    closeAll();
    document.body.classList.add("no_scroll");
    document.body.classList.add("no-hover"); // Disable hover when gallery is active
    const gallery = document.getElementById('gallery_' + id);
    if (gallery) {
        gallery.classList.add('gallery_active');
        isGalleryUpdating = true;  // Disable hover effect while gallery is being updated
...
function closeAll() {
    ...
    document.body.classList.remove("no-hover"); // Enable hover when gallery is closed
    const activeGallery = document.querySelector('.gallery_active');
    if (activeGallery) {
        activeGallery.classList.remove('gallery_active');
    }
    isGalleryUpdating = false;  // Re-enable hover effect when gallery is closed
}
...
document.querySelectorAll(".item").forEach(img => {
    ...
    img.addEventListener("mouseenter", () => {
        if (!isGalleryUpdating) {  // Only change on hover if gallery isn't updating
            img.src = gifSrc;
        }
    });

    img.addEventListener("mouseleave", () => {
        if (!isGalleryUpdating) {  // Only reset on hover if gallery isn't updating
            img.src = "";
            setTimeout(() => {
                img.src = originalSrc;
            }, 10);
