...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const banner = document.querySelector('.banner'); // 选取 banner
    ...
    fadeIns.forEach(fadeIn => fadeObserver.observe(fadeIn));

    // 监听 banner 进入视口，让 fade-in 消失
    const bannerObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                fadeIns.forEach(fadeIn => fadeIn.classList.add('hidden')); // 添加隐藏类
            } else {
                fadeIns.forEach(fadeIn => fadeIn.classList.remove('hidden')); // 退出 banner 视图时恢复
            }
        });
    }, {
        root: null,
        threshold: 0.5, // 调高 threshold，确保 banner 足够进入视口时触发
        rootMargin: "-20% 0px" // 让触发更早发生
    });
