...
document.addEventListener("DOMContentLoaded", function () {
    ...
    console.log("Buttons length:", buttons.length);  // 检查是否获取到按钮
    console.log("Items length:", items.length);  // 检查是否获取到项目
    ...
    for (let i = 0; i < buttons.length; i++) {
        ...
        button.addEventListener('click', function () {
            ...
            console.log(`Button clicked: ${category}`);  // 输出被点击的按钮类别
            ...
            for (let j = 0; j < items.length; j++) {
                ...
                if (category === 'all' || itemCategory === category) {
                    ...
                    console.log(`Displaying: ${itemCategory}`); // 输出显示的项目
                } else {
                    item.style.display = 'none';
                    console.log(`Hiding: ${itemCategory}`);  // 输出隐藏的项目
                }
            }
        });
    }
