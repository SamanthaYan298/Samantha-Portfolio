...
document.addEventListener('DOMContentLoaded', () => {
    ...
    buttons.forEach(button => {
        ...
        button.addEventListener('click', (event) => {
            const category = event.target.getAttribute('data-category'); // 获取按钮的类别

            const items = document.querySelectorAll('.ag-courses_item'); // 获取所有项目
            items.forEach(item => {
                const itemCategory = item.getAttribute('data-category'); // 获取项目的类别
