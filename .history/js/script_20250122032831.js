...
const star = document.querySelector('.floating-star');
...
window.addEventListener('scroll', function() {
    ...
    if (scrollTop + window.innerHeight >= footerOffset) {
        star.style.position = 'absolute';  // 改为绝对定位
        star.style.bottom = '0';  // 停在footer上方
    } else {
        star.style.position = 'fixed';  // 改为固定定位
        star.style.bottom = '6rem';  // 默认位置
    }
});
