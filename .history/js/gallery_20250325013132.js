...
function updateGallery(imageUrl, selectedImage, galleryId) {
    const activeGallery = document.getElementById('gallery_' + galleryId); // 根据 galleryId 查找当前画廊
    if (activeGallery) {
        const mainImage = activeGallery.querySelector(`#gallery_display_${galleryId}`); // 根据 galleryId 获取 mainImage
