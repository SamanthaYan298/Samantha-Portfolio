...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const bannerObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                floatingStar.classList.add('hidden'); // banner 可见时隐藏按钮
            } else {
                floatingStar.classList.remove('hidden'); // banner 不可见时恢复按钮
            }
        });
    }, {
        root: null,
        threshold: 0.1  // 当 10% 的 banner 可见时触发
    });
    ...
    const footerObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                floatingStar.classList.add('at-footer'); // 到达 footer 时，确保按钮停在 footer 上方
            } else {
                floatingStar.classList.remove('at-footer'); // 离开 footer 区域时恢复正常位置
            }
        });
    }, {
        root: null,
        threshold: 0.1  // 当 10% 的 footer 可见时触发
    });
