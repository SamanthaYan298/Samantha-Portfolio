    ...
    const bannerObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                console.log('Banner is visible!'); // ✅ 测试 IntersectionObserver 是否生效
                fadeIns.forEach(fadeIn => fadeIn.classList.add('hidden'));
            } else {
                console.log('Banner is not visible.'); // ✅ 退出时打印
                fadeIns.forEach(fadeIn => fadeIn.classList.remove('hidden'));
            }
        });
    }, observerOptions);
